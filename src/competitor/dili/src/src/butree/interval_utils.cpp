...
//            }
    ...
    cout << min_size << " " << border_set.size() << endl;
...
//        cout << "######, xcount = " << xcount << endl;
        ...
        long est_n_nodes = h0_borders.size();
...
//        cout << "curr_h = " << curr_h << ", root_loss = " << root_loss << endl;
        ...
        if (!restore_status) {
            cout << "in h1, when try to get_complete_partition_borders" << endl;
            // get_complete_partition_borders(topX, nullptr, curr_n_nodes, curr_h, 100, fanThreashold/2, hi_borders, hi_rmses, interval_type);
            get_complete_partition_borders(topX, nullptr, curr_n_nodes, curr_h, buMinFan, fanThreashold/2, hi_borders, hi_rmses, interval_type);
            
            cout << "in h1, when got_complete_partition_borders" << endl;
//            get_complete_partition_borders(topX, NULL, curr_n_nodes, curr_h, buMinFan * 2, fanThreashold/2, hi_borders, hi_rmses, interval_type);
            string hi_borders_path = mirror_dir + "/h" + std::to_string(curr_h) + "_borders";
            data_utils::save_vec_data(hi_borders_path.c_str(), hi_borders);

            string hi_rmses_path = mirror_dir + "/h" + std::to_string(curr_h) + "_rmses";
            data_utils::save_vec_data(hi_rmses_path.c_str(), hi_rmses);
        }
        ...
        long est_next_n_nodes = 0;
        double hi_loss = 0;
        cout << "lastly, try to estimate_ideal_layout" << endl;
        ...
        long est_n_nodes = hi_borders.size();
        cout << "lastly, got estimate_ideal_layout" << endl;
