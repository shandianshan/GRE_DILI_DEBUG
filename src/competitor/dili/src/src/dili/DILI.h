...
//    root->set_range(0, all_keys[N-1] + 1);
        ...
        root->a = -(root->b * lbd);
        root->node_mid_range = n_keys/2;
        root->node_mid_key = split_keys_list[H-1][n_keys/2];
...
//        std::cout << "******key = " << key << std::endl;
        ...
        while (true) {
            ...
            int pred = std::min<long long>(std::max<long long>(( node->node_mid_range + node->b * (key - node->node_mid_key)), 0), node->fanout - 1);
