...
struct diliNode{
    keyType node_mid_key;
    int node_mid_range;
    double node_expan_ratio; 
...
//        cout << "pe.key = " << pe.key << endl;
    ...
    inline diliNode* find_child(const keyType &key) {
        ...
        int i = std::min<long long>(std::max<long long>(( node_mid_range + b * (key - node_mid_key)), 0), fanout - 1);
    ...
    inline void put_three_keys(const keyType &k0, const recordPtr &p0, const keyType &k1, const recordPtr &p1, const keyType &k2, const recordPtr &p2) {
        ...
        a = 0.5 + offset - b * k1;
        node_mid_key = k1;
        node_mid_range = offset;

        int pos0 = std::min<long long>(std::max<long long>(static_cast<long long >(offset + b * (k0 - k1)), 0), fanout - 1);
        int pos1 = std::min<long long>(std::max<long long>(static_cast<long long >(offset + b * (k1 - k1)), 0), fanout - 1);
        int pos2 = std::min<long long>(std::max<long long>(static_cast<long long>( offset + b * (k2 - k1)), 0), fanout - 1);
    ...
    inline void put_three_keys(const keyType *_keys, const recordPtr *_ptrs) {
        ...
        node_mid_key = k1;
        node_mid_range = offset;
        // cout << "fanout " << fanout << " s " << s << " b " << b << " a " << a << endl;
        // double re_tem = a+ b*k1;
        // cout << re_tem << endl;
        // int pos0 = LR_PRED(a, b, k0, fanout);
        // int pos1 = LR_PRED(a, b, k1, fanout);
        // int pos2 = LR_PRED(a, b, k1, fanout);
        // int pos0 = std::min<int>(std::max<int>(static_cast<int>(0.5 + offset + b * (k0 - k1)), 0), fanout - 1);
        int pos0 = std::min<long long>(std::max<long long>(static_cast<long long >(offset + b * (k0 - k1)), 0), fanout - 1);
        int pos1 = std::min<long long>(std::max<long long>(static_cast<long long >(offset + b * (k1 - k1)), 0), fanout - 1);
        int pos2 = std::min<long long>(std::max<long long>(static_cast<long long>( offset + b * (k2 - k1)), 0), fanout - 1);
        // cout << "b*(k2 -k1) " << static_cast<long long>(b*(k2-k1)) << endl;
...
//                pe_data = new pairEntry[fanout];
    ...
    void linearReg_w_simple_strategy_reimplement(const keyType *X, long long &l_a, long long &l_b, int n, int scal_long){
        int left_n = n / 2;
        int right_n = n - left_n - 1;
        keyType x_middle = X[left_n];
        double left_slope = 1.0 * left_n / (x_middle - X[0]);
        double right_slope = 1.0 * right_n / (X[n-1] - x_middle);
        double b = MAX_DOUBLE(left_slope, right_slope);
        l_b = (long long) (b*scal_long);
        if(l_b == 0){
            l_b = 1;
        }
        l_a = - l_b * x_middle + scal_long* left_n;
    }
    ...
    void distribute_data(const keyType *keys, const recordPtr *ptrs, bool print=false) {
        ...
        node_mid_key = keys[num_nonempty/2];
        node_mid_range = num_nonempty/2;
        if (fanout > num_nonempty) {
            node_mid_range = fanout/2;
        }
        // store b, the middle key, the middle range
        ...
        int last_pos = std::min<long long>(std::max<long long>(( node_mid_range + b * (last_key-node_mid_key)), 0), fanout - 1);
        int final_pos = std::min<long long>(std::max<long long>(( node_mid_range+ b * (final_key-node_mid_key)), 0), fanout - 1);
    ...
    void distribute_data1(const keyType *keys, const recordPtr *ptrs, bool print=false) {
        assert(num_nonempty > 3);
